...
このファイル自体は
[SIL Open Font License 1.1](packages/targets/assets/fonts/OFL.txt) のままです。
zip 自体にはライセンスファイルが入っていないため、zip を第三者に渡すときは
[`OFL.txt`](packages/targets/assets/fonts/OFL.txt) のコピーを添えてください。
このファイル1つに、OFL 第2条件が各コピーへの同梱を義務づける著作権表示と
ライセンス本文の両方が含まれています。また、フォントファイル単体を販売する
ことはできません（OFL 第1条件）。自分で登録したフォントの場合は、その
フォントのライセンス条件に従ってください。
...
出力ランタイム、既製テンプレート集を計画しています。いずれも現時点ではまだ
存在せず、実装され次第この README を更新します。
